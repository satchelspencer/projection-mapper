(function($){
	...
	$.fn.projectionMap = function(coords, or){
		...
		var init = [[or[0],or[1]],[or[2],or[1]],[or[0],or[3]],[or[2],or[3]]];
	...
	$.fn.projectionMapGui = function(){
		...
		var origin = [left, top, this.width()+left, this.height()+top];
		var coo = [origin[0], origin[1], origin[2], origin[1], origin[0], origin[3], origin[2], origin[3]];
		var v = ['top', 'bottom'];
		var h = ['left', 'right'];
		var index = 0;
		...
		function getHandle(index){
			return $('<div>').css(css).mousedown(function(){	
				...
				$(window).on('mouseup', function(){$(window).off('mousemove mouseup')});
			});
		}
	}
}(jQuery));